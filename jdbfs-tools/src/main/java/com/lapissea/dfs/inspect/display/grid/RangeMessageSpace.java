...
public class RangeMessageSpace{
	...
	private record Node(Range range, ArrayList<Node> children, Message val) implements Comparable<Node>{
		...
		private void doRelax(){
			...
			var split = ch.size()/2;
			var l1    = ch.subList(0, split);
			var l2    = ch.subList(split, ch.size());
			
			children.clear();
			children.add(new Node(calcRange(l1), Iters.from(l1).map(Node::new).asCollection(ArrayList::new), null));
			children.add(new Node(calcRange(l2), Iters.from(l2).map(Node::new).asCollection(ArrayList::new), null));
			children.trimToSize();
		}
		
		private Range calcRange(List<Message> l1){
			return new Range(
