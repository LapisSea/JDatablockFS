...
public interface ContentReader extends AutoCloseable, ContentBuff{
	...
	default char[] readChars(int elementsToRead) throws IOException{
		...
		while(remaining>0){
			...
			for(int i=0;i<readElements;i++){
				result[read+i]=readChar(buff, i*bytesPerElement);
			}
	...
	default char readChar() throws IOException{
		...
		return readChar(readBuffer, 0);
	}
	private char readChar(byte[] readBuffer, int offset){
		...
		return new ContentInputStream.BA(readInts1(amount)){
			...
			public void close() throws IOException{
				...
				if(errorOnMismatch!=null){
					var av=available();
					if(av>0) throw new BufferUnderflowException();
				}
			}
		};
	}
